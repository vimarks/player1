...
export class Bullet extends Moving {
  constructor(state, offsetX, offsetY, rotation, velocityX, velocityY) {
    ...
    let newVelocityX =
      velocityX + Math.sin(rotation) * constants.bulletVelocity;
    let newVelocityY =
      velocityY + -Math.cos(rotation) * constants.bulletVelocity;
  ...
  // bullet isremoved from stage after a period of time (bulletDecay)
  start(parentNode) {
    super.start(parentNode);
    setTimeout(() => {
      this.node.remove();
