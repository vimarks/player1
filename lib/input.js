 ...
 *
 * These events are intended to be compatible with the ActionListener class
...
export class Input {
  ...
  setAction(stage, action, value = ACTIVE) {
    if (action && this.actions[action] !== value) {
      ...
      stage.publish("Input.action." + action, [this.now(), value]);
    }
  }
  ...
  clearAction(stage, action) {
    if (action && this.actions[action]) {
      ...
      stage.publish("Input.action." + action + ".clear", [this.now()]);
    }
  }
