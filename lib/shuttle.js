...
export class Shuttle extends Moving {
  constructor(actions) {
    ...
    this.actions = new ActionListener(this.node, [
      "fire",
      "focus",
      "turnLeft",
      "turnRight",
  ...
  tick(dt, stage) {
    ...
    let focus = this.actions.get("focus");
    if (focus) {
      let focusAngle = this.getFocusAngle(focus);
    ...
    if (this.actions.get("turnLeft") && !this.actions.get("turnRight")) {
      this.rotateBy(constants.shuttleRotation, dt);
    } else if (this.actions.get("turnRight")) {
      this.rotateBy(-constants.shuttleRotation, dt);
    }

    // Engage thrusters with the 'accelerate' action
    if (this.actions.get("accelerate")) {
      this.accelerate(
        this.rotation,
